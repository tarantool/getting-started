...
Она называется **\"Reset Configuration\".** Она находится сверху, во вкладке "Cluster".
...
Здесь мы сможем создать файл под названием `schema.yml`.
В нем можно описать схему данных для всего кластера,
...
**Чтобы загрузить схему в кластер создайте файл `schema.yml`. Скопируйте и вставьте
схему в этот файл. Нажмите на кнопку "Apply". После этого, в кластере будет описана схема данных.**
...
**Процедуры нужно описать в специальном файле. Для этого перейдите во вкладку "Code". Создайте
новую дирректорию под названием "extensions". И в этой дирректории создайте файл "api.lua".**
...
**Чтобы сконфигурировать HTTP пути необходимо написать конфигурационный файл. Для этого перейдите во вкладку "Code". Создайте
файл "config.yml" в дирректории "extensions". Вы ее создавали на прошлом шаге.**
...
Обратите внимание: инструмент `space-explorer` доступен только в Enterprise версии продукта.
В open-source версии данные можно посмотреть через консоль.

Читайте [подробнее в документации](https://www.tarantool.io/ru/doc/latest/reference/reference_lua/box_space/select/).
...
![Cluster, просмотр узла s2-master](images/scaling.png)
...
- склонируйте репозиторий [https://github.com/tarantool/getting-started](https://github.com/tarantool/getting-started)
